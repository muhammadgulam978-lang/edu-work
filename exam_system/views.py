...
from exam_system.models import (
    ...
    CentralizedResult,
    GradingScale, GradeBand, AssessmentComponent, TermWeightage,
)
from exam_system.services import (
    ...
    compile_results,
    validate_component_weightage,
    validate_term_weightage,
)
...
# =============================================================
# SECTION 9B: GRADING SCALE + WEIGHTAGE CONFIG
# (E-Results workflow foundation — Section 5 & 10)
# =============================================================

@login_required
def grading_scale_list(request):
    """Active academic year ke saare grading scales list karo."""
    academic_year = get_active_year()
    scales = GradingScale.objects.filter(
        academic_year=academic_year
    ).prefetch_related('bands') if academic_year else GradingScale.objects.none()

    return render(request, 'exam_system/grading_scale_list.html', {
        'academic_year': academic_year,
        'scales': scales,
    })
...
@login_required
def create_grading_scale(request, scale_id=None):
    """Naya grading scale banao ya existing edit karo (bands ke saath)."""
    if not request.user.is_staff:
        messages.error(request, "Only an admin can configure grading scales.")
        return redirect('grading_scale_list')

    academic_year = get_active_year()
    scale = get_object_or_404(GradingScale, id=scale_id) if scale_id else None
    ...
    if request.method == 'POST':
        ...
        if not name:
            messages.error(request, "Scale ka naam required hai.")
            return redirect('grading_scale_list')

        if scale is None:
            scale = GradingScale(academic_year=academic_year, created_by=request.user)
        scale.name       = name
        ...
        mins    = request.POST.getlist('min_percent')
        maxs    = request.POST.getlist('max_percent')
        ...
        for i in range(len(grades)):
            ...
            GradeBand.objects.create(
                scale=scale,
                grade=grades[i],
                min_percent=mins[i] or 0,
                max_percent=maxs[i] or 0,
                grade_point=points[i] or 0,
                description=descs[i] if i < len(descs) else '',
            )

        messages.success(request, f"Grading scale '{scale.name}' saved.")
        return redirect('grading_scale_list')

    return render(request, 'exam_system/create_grading_scale.html', {
        'academic_year': academic_year,
        'scale': scale,
    })
...
def delete_grading_scale(request, scale_id):
    ...
    if request.user.is_staff:
        scale.delete()
        messages.success(request, "Grading scale deleted.")
    return redirect('grading_scale_list')


@login_required
def term_weightage_config(request, class_id):
    """
    Ek class ke liye, active academic year ke saare exam terms (ExamPlan)
    ka annual result mein weightage % set/edit karo.
    """
    ...
    if request.method == 'POST':
        if not request.user.is_staff:
            messages.error(request, "Only an admin can configure weightage.")
            return redirect('term_weightage_config', class_id=class_obj.id)

        for plan in plans:
            value = request.POST.get(f'weightage_{plan.id}')
            if value not in (None, ''):
                TermWeightage.objects.update_or_create(
                    class_fk=class_obj, academic_year=academic_year, exam_plan=plan,
                    defaults={'weightage_percent': value}
                )

        is_valid, total = validate_term_weightage(class_obj, academic_year)
        if is_valid:
            messages.success(request, f"Term weightage saved. Total = {total}%.")
        else:
            messages.warning(request, f"Saved, but total weightage is {total}% — should equal 100%.")
        return redirect('term_weightage_config', class_id=class_obj.id)

    existing_map = {tw.exam_plan_id: tw.weightage_percent for tw in TermWeightage.objects.filter(
        class_fk=class_obj, academic_year=academic_year
    )}
    plan_rows = [
        {'plan': plan, 'weightage': existing_map.get(plan.id, '')}
        for plan in plans
    ]
    is_valid, total = validate_term_weightage(class_obj, academic_year)

    return render(request, 'exam_system/term_weightage_config.html', {
        'class_obj': class_obj,
        'academic_year': academic_year,
        'plan_rows': plan_rows,
...
@login_required
def assessment_component_config(request, plan_id, subject_id):
    """
    Ek exam term + subject ke andar assessment components
    (Classwork, Quizzes, Midterm ...) aur unka weightage % set karo.
    """
    plan    = get_object_or_404(ExamPlan, id=plan_id)
    subject = get_object_or_404(Subject, id=subject_id)
    ...
    if request.method == 'POST':
        ...
        AssessmentComponent.objects.filter(exam_plan=plan, subject=subject).delete()
        names   = request.POST.getlist('component_name')
        weights = request.POST.getlist('weightage_percent')
        maxm    = request.POST.getlist('max_marks')

        for i in range(len(names)):
            if not names[i]:
                continue
            AssessmentComponent.objects.create(
                exam_plan=plan, subject=subject,
                name=names[i],
                weightage_percent=weights[i] or 0,
                max_marks=maxm[i] or 100,
            )

        is_valid, total = validate_component_weightage(plan, subject)
        if is_valid:
            messages.success(request, f"Weightage saved. Total = {total}%.")
        else:
            messages.warning(request, f"Saved, but total weightage is {total}% — should equal 100%.")
        return redirect('assessment_component_config', plan_id=plan.id, subject_id=subject.id)

    components = AssessmentComponent.objects.filter(exam_plan=plan, subject=subject)
    is_valid, total = validate_component_weightage(plan, subject)

    return render(request, 'exam_system/assessment_component_config.html', {
        'plan': plan,
        'subject': subject,
