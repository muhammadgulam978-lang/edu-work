...
# =============================================================
# SECTION 3B: GRADING SCALE HELPERS
# (Section 5 & 10 of E-Results workflow — configurable scale + weightage)
# =============================================================

def get_grading_scale_for(academic_year):
    """
    Diye gaye academic year ke liye default GradingScale return karta hai.
    Agar koi scale configured nahi hai to None deta hai — caller ko
    hardcoded fallback use karna chahiye taake purane data pe asar na pade.
    """
    from exam_system.models import GradingScale
    return GradingScale.objects.filter(
        academic_year=academic_year, is_default=True
    ).first() or GradingScale.objects.filter(academic_year=academic_year).first()


def resolve_grade(percentage, academic_year=None, scale=None):
    """
    Percentage ko grade + grade_point mein convert karta hai.
    Pehle configured GradingScale check karta hai; agar na mile to
    original hardcoded scale (A+ se F tak) fallback ke taur pe use hoti hai,
    taake purani results / academic years break na hon.
    Returns: (grade:str, grade_point:float or None)
    """
    if scale is None and academic_year is not None:
        scale = get_grading_scale_for(academic_year)

    if scale is not None:
        band = scale.get_band_for_percentage(percentage)
        if band is not None:
            return band.grade, float(band.grade_point)

    # ── Fallback: hardcoded default scale ─────────────────────
    if   percentage >= 90: return 'A+', 4.0
    elif percentage >= 80: return 'A',  3.7
    elif percentage >= 70: return 'B',  3.0
    elif percentage >= 60: return 'C',  2.5
    elif percentage >= 50: return 'D',  2.0
    elif percentage >= 40: return 'E',  1.0
    else:                  return 'F',  0.0


def validate_component_weightage(exam_plan, subject):
    """
    Ek subject ke assessment components (Classwork, Quiz, Midterm...) ka
    total weightage 100% hona chahiye. Returns (is_valid, total).
    """
    from exam_system.models import AssessmentComponent
    total = AssessmentComponent.total_weightage(exam_plan, subject)
    return (float(total) == 100.0, total)


def validate_term_weightage(class_fk, academic_year):
    """
    Ek class ke annual result mein har exam term (First/Second/Final ...)
    ka total weightage 100% hona chahiye. Returns (is_valid, total).
    """
    from exam_system.models import TermWeightage
    total = TermWeightage.total_weightage(class_fk, academic_year)
    return (float(total) == 100.0, total)
...
def compile_results(schedule) -> int:
    ...
    academic_year = schedule.exam_plan.academic_year
    scale = get_grading_scale_for(academic_year)
    ...
    for sheet in sheets:
        ...
        grade, _grade_point = resolve_grade(percentage, scale=scale)
