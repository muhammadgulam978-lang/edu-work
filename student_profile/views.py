...
# =============================================================
# E+ RESULT — GRADE PREDICTOR (student view)
# Projects the student's likely Final Term % per subject from their
# Midterm result and the class-wide Midterm -> Final trend for that
# subject. Same logic as the teacher-side predictor, scoped to one
# student, so both sides always agree on the number.
# =============================================================
@login_required
def student_grade_predictor(request):
    user = request.user
    try:
        student = Student.objects.get(user=user)
    except Student.DoesNotExist:
    ...
    for subject in subjects:
        ...
        for res in class_results:
            if not res.total_marks:
                continue
            try:
                pct = round((float(res.marks_obtained) / float(res.total_marks)) * 100, 1)
        ...
        my_midterm = midterm.get(student.id)
        my_final = final.get(student.id)

        if my_final is not None:
            # already have the real result — nothing to predict
            rows.append({
                'subject': subject, 'midterm_pct': my_midterm,
                'final_pct': my_final, 'predicted_pct': None, 'trend': None,
            })
            continue

        if my_midterm is None:
            rows.append({
                'subject': subject, 'midterm_pct': None,
                'final_pct': None, 'predicted_pct': None, 'trend': None,
            })
            continue

        deltas = [final[sid] - midterm[sid] for sid in midterm if sid in final]
        avg_delta = (sum(deltas) / len(deltas)) if deltas else 0.0
        predicted = max(0.0, min(100.0, my_midterm + avg_delta))

        rows.append({
            'subject': subject,
            'midterm_pct': my_midterm,
            'final_pct': None,
            'predicted_pct': round(predicted, 1),
            'trend': 'Improving' if avg_delta > 1 else ('Declining' if avg_delta < -1 else 'Stable'),
        })

    return render(request, "student_profile/grade_predictor.html", {
        "student": student,
        "rows": rows,
    })
