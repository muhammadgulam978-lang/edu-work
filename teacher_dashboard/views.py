...
# =============================================================
# E+ RESULT — hub, validation summary and AI-style analysis
# (built on the existing ExamResult data — admin_panel model,
#  same one already used by upload_result / merge_result /
#  student_result, so nothing new to migrate)
# =============================================================

def _eresult_pct(res):
    """Safe percentage helper for an ExamResult row."""
    try:
        if res and res.total_marks:
            return round((float(res.marks_obtained) / float(res.total_marks)) * 100, 1)
    except (TypeError, ValueError, ZeroDivisionError):
        pass
    return None


def _eresult_validation(class_id, section_id, subject_id, term):
    """Smart-validation-lite: missing / zero / above-max / outlier marks."""
    students = Student.objects.filter(class_fk_id=class_id, section_id=section_id)
    results = {
        r.student_id: r for r in ExamResult.objects.filter(
            class_fk_id=class_id, section_id=section_id,
            subject_id=subject_id, term=term,
        )
    }
    ...
    for student in students:
        ...
        if not res:
            issues.append({'student': student, 'issue': 'Missing marks'})
            continue
        entered += 1
        if res.total_marks and res.marks_obtained is not None:
            try:
                if float(res.marks_obtained) > float(res.total_marks):
                    issues.append({'student': student, 'issue': 'Marks above maximum'})
                elif float(res.marks_obtained) == 0:
                    issues.append({'student': student, 'issue': 'Zero marks — please confirm'})
    ...
    return {
        ...
        'entered': entered,
        'missing': students.count() - entered,
        'issues': issues,
    }


@login_required
def eresult_hub(request):
    """
    E+ Result — teacher landing page.
    Shows every assigned class/section/subject with a live marks-entry
    status and quick links into Marks Entry, Class Result and Grade
    Predictor — mirrors steps 1-5 of the E+ Result workflow using the
    data that is already being captured today.
    """
    teacher = get_object_or_404(Teacher, user=request.user)
    term = request.GET.get('term') or 'midterm'

    assigned_periods = AssignedPeriod.objects.filter(teacher=teacher).values(
        'class_fk__id', 'class_fk__class_name',
        'section__id', 'section__section_name',
        'subject__id', 'subject__name',
    ).distinct()

    rows = []
    for ap in assigned_periods:
        summary = _eresult_validation(
            ap['class_fk__id'], ap['section__id'], ap['subject__id'], term
        )
        if summary['total_students'] == 0:
            status = 'No students'
        elif summary['missing'] == 0 and not summary['issues']:
            status = 'Verified'
        elif summary['entered'] == 0:
            status = 'Not Started'
        else:
            status = 'Draft'
        rows.append({**ap, 'summary': summary, 'status': status})

    class_teacher_sections = ClassTeacher.objects.filter(teacher=teacher).select_related(
        'class_fk', 'section'
    )

    return render(request, 'teacher_dashboard/eresult_hub.html', {
        'teacher': teacher,
        'term': term,
        'rows': rows,
...
@login_required
def eresult_class_analysis(request, class_id, section_id, subject_id):
    """
    E+ Result — subject-level analysis for a single class/section.
    Rule-based version of the 'AI Result Analysis' step: class average,
    highest/lowest, pass rate, grade spread and flagged students.
    """
    teacher = get_object_or_404(Teacher, user=request.user)
    term = request.GET.get('term') or 'midterm'
    ...
    validation = _eresult_validation(class_id, section_id, subject_id, term)
    ...
    for res in results:
        ...
        student_rows.append({
            ...
            'at_risk': pct is not None and pct < 40,
        })

    student_rows.sort(key=lambda r: (r['pct'] is None, -(r['pct'] or 0)))

    class_avg = round(sum(percentages) / len(percentages), 1) if percentages else None
    ...
    pass_rate = round((pass_count / len(percentages)) * 100, 1) if percentages else None
    at_risk_count = len([r for r in student_rows if r['at_risk']])

    return render(request, 'teacher_dashboard/eresult_class_analysis.html', {
        'teacher': teacher, 'term': term,
        'class_obj': class_obj, 'section_obj': section_obj, 'subject_obj': subject_obj,
        'validation': validation,
        'student_rows': student_rows,
        'class_avg': class_avg, 'highest': highest, 'lowest': lowest,
        'pass_rate': pass_rate, 'at_risk_count': at_risk_count,
    })


def _grade_prediction_rows(class_id, section_id, subject_id):
    """
    Grade Predictor core logic.
    Uses the class-wide midterm -> final movement (where both exist) as
    the trend, then applies that trend to students who only have a
    midterm result yet, to project their likely final-term percentage.
    """
    results = ExamResult.objects.filter(
        class_fk_id=class_id, section_id=section_id, subject_id=subject_id,
    ).select_related('student')
    ...
    for res in results:
        ...
        if pct is None:
            continue
        if res.term == 'midterm':
            midterm[res.student_id] = pct
        elif res.term == 'final':
            final[res.student_id] = pct

    deltas = [final[sid] - midterm[sid] for sid in midterm if sid in final]
    avg_delta = round(sum(deltas) / len(deltas), 1) if deltas else 0.0

    rows = []
    for sid, m_pct in midterm.items():
        if sid in final:
            continue  # final already recorded, no need to predict
        predicted = max(0.0, min(100.0, m_pct + avg_delta))
        rows.append({
            'student': next((r.student for r in results if r.student_id == sid), None),
            'midterm_pct': m_pct,
            'predicted_final_pct': round(predicted, 1),
            'trend': 'Improving' if avg_delta > 1 else ('Declining' if avg_delta < -1 else 'Stable'),
            'at_risk': predicted < 40,
        })

    rows.sort(key=lambda r: r['predicted_final_pct'])
    return rows, avg_delta, len(deltas)


@login_required
def grade_predictor_hub(request):
    """Grade Predictor — pick a class/section/subject to predict."""
    teacher = get_object_or_404(Teacher, user=request.user)
    assigned_periods = AssignedPeriod.objects.filter(teacher=teacher).values(
        'class_fk__id', 'class_fk__class_name',
        'section__id', 'section__section_name',
    ...
    return render(request, 'teacher_dashboard/grade_predictor_hub.html', {
        'teacher': teacher,
        'assigned_periods': assigned_periods,
    })
...
@login_required
def grade_predictor_detail(request, class_id, section_id, subject_id):
    class_obj = get_object_or_404(Class, id=class_id)
    section_obj = get_object_or_404(Section, id=section_id)
    subject_obj = get_object_or_404(Subject, id=subject_id)

    rows, avg_delta, sample_size = _grade_prediction_rows(class_id, section_id, subject_id)

    return render(request, 'teacher_dashboard/grade_predictor_detail.html', {
        'class_obj': class_obj, 'section_obj': section_obj, 'subject_obj': subject_obj,
        'rows': rows, 'avg_delta': avg_delta, 'sample_size': sample_size,
    })
